...
public class GlobalExceptionHandler {
    ...
    /**
     * Devolve a mensagem específica da falha de IA (parse JSON, modelo, timeout, etc.).
     * Antes mascarava tudo como "Tutor IA indisponível", o que impedia diagnóstico
     * quando o Ollama estava ligado mas a resposta era inválida.
     */
    ...
    public ResponseEntity<ApiError> handleIAIndisponivel(IAIndisponivelException ex, HttpServletRequest req) {
        log.error("Falha de IA em {}: {}", req.getRequestURI(), ex.getMessage());
        String msg = ex.getMessage() != null && !ex.getMessage().isBlank()
                ? ex.getMessage()
                : "O serviço de IA falhou. Tenta novamente dentro de instantes.";
